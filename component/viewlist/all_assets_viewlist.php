...
if (!empty($_POST['_id'])) {
    ...
    $and = "a.is_deleted = '0' and a.category = '$_id'";
} else {
    $and = "a.is_deleted = '0'";
}
if (!empty($_POST['sid'])) {
        ...
        <?php if ($row['assigned_user'] == '0' && $row['asset_assign'] == '') { ?>
            ...
            <td>
                ...
                <?php } else if ($row['assigned_user'] != '' && $row['assign_method'] == 1) { ?>
                    <a href="#" data-toggle="modal" data-target="#checkinconfirm" onclick="checkinconfirm('<?= $row['component_uid']; ?>','<?= $row['assigned_user']; ?>','<?= $row['asset_assign']; ?>')"><i class="fas fa-user" title="Checkin"></i></a>
                <?php } ?>
